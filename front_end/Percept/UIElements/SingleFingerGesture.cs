...
namespace Percept.UIElements
{
    public class SingleFingerGesture : Gesture
	{
		...
		override protected void UpdateGesture(object state)
		{
			...
			InvokeOnMainThread(() =>
			{
				...
				if (translationThresholdPassed)
				{
                    ...
                    var sensor = virtualObject as SensorDisplay;
                    //if(sensor!= null)
                    //{
                    //    if(sensor.p != null)
                    //    {
                    //        sensor.p.RemoveFromParentNode();
                    //        sensor.p = null;
                    //    }
                    //}
        ...
        {
            Debug.Print("SingleFingerGesture FinishGesture()");
            ...
            if (hasMovedObject)
            {
                manager.VirtualObjectManagerDelegate.TranslationFinishedFor(LastUsedObject);
                return;
            }

            var vo = VirtualObjectAt(latestTouchPosition);
            if (vo == null)
            {
                manager.VirtualObjectManagerDelegate.NothingTapped();
            } else
            {
                Debug.Print("SingleFingureGesture calling IsTouched()");
                if (manager.VirtualObjectManagerDelegate != null)
                {
                    manager.VirtualObjectManagerDelegate.ObjectTapped(vo);
                }
            }
            ...
            //		if (!translationThresholdPassed)
            //		{
            //			manager.Translate(LastUsedObject, SceneView, latestTouchPosition, true, true);
            //		}
            //             }
            //             else
            //             {
            //                 
            //                 //if we did hit an object then we are selecting it.
            //                 Debug.Print("SingleFingureGesture calling IsTouched()");
            //                 if (manager.VirtualObjectManagerDelegate != null)
            //                 {
            //                     manager.VirtualObjectManagerDelegate.ObjectTapped(vo);
