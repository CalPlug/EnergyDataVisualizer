...
namespace Percept.UIElements
{
	public abstract class Gesture : NSObject
	{
		...
		protected VirtualObject VirtualObjectAt(CGPoint loc)
		{
            ...
            try
            {
                var firstNodeHit = hitTestResults.First((result) => VirtualObject.GetParentOfType<VirtualObject>(result.Node) != null);
                var voNode = VirtualObject.GetParentOfType<VirtualObject>(firstNodeHit.Node);
                return voNode as VirtualObject;
            }catch(Exception e)
            {
                Debug.Print("VirtualObjectAt" + e.Message);
                return null;
            }
		}
