...
namespace Percept.UIElements
{
	public class TextManager : NSObject
	{
		...
		public IARViewControllerDelegate Controller { get; set; }

		public TextManager(IARViewControllerDelegate controller)
		{
			// Initialize
		...
		public void ShowMessage(string text, bool autoHide = true)
		{
			InvokeOnMainThread(() =>
			{
				...
				InvokeOnMainThread(() =>
                {
                    UILabel label = Controller.GetMessageLabel();
                    if(label != null)
                    {
                        label.Text = text;
                    }
		...
		public void BlurBackground()
		{
			InvokeOnMainThread(() =>
			{
				...
				{
					Frame = Controller.GetView().Bounds,
					AutoresizingMask = UIViewAutoresizing.FlexibleWidth | UIViewAutoresizing.FlexibleHeight,
					Tag = BlurEffectViewTag
				};
				Controller.GetView().AddSubview(blurEffectView);
			});
		}
		...
		public void UnblurBackground()
		{
			...
			{
				// Scan all views
				foreach (UIView view in Controller.GetView().Subviews)
				{
		...
		private void UpdateMessagePanelVisibility()
		{
			InvokeOnMainThread(() =>
			{
                ...
                if(label != null)
                {
                    var anyMessageShown = !label.Hidden;

                    // Only show message panel if something is visible
                    label.Hidden = !anyMessageShown;
                }
